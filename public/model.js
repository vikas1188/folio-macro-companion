...
export function policyIllustration(value,weights,probabilities){return signalIllustration(value,weights,probabilities,'fed');}
export function signalIllustration(value,weights,probabilities,signal){
  if(!Object.hasOwn(signals,signal))throw new Error('Unknown signal');
  const returns=signals[signal].returns;
  ...
  const outcomes=Object.fromEntries(keys.map(k=>[k,weights.reduce((s,w,i)=>s+value*w/100*returns[k][i]/100,0)]));
